    ...
    if ans=="1":
            ...
            if y is not None:
                ...
                while True:
                    ...
                    if a==1:
                        ...
                        mycur.execute("select * from ChemInv where name=\"{0}\";".format(Chemicalname))
                        row=mycur.fetchone()
                        if row is not None:
                            print("Chemical already exists!! ")
                        else:
                            mycur.execute("insert into ChemInv(name,casnum,price,unit,qty,lstockqty) values(\"{0}\",\"{1}\",{2},\"{3}\",{4},{5}); ".format(Chemicalname,casnum,price,unit,quantity,lstockqty))
                            mycon.commit()
                        ...
                        row=mycur.fetchone()
                        if row is not None:
                            mycur.execute("update ChemInv set qty={0} where name=\"{1}\";".format(quantity,Chemicalname))
                            mycon.commit()
                        ...
                        mycur.execute("select * from ChemInv; ")
                        x=mycur.fetchall()
                        for i in x:
                            print(i)
                        ...
                        Sprice=n*price
                        mycur.execute("select qty from ChemInv where Id={0};".format(Id))
                        lk=mycur.fetchone()
                        if max(lk)<n:
                            print(n,"Chemical Quantity required is not available!!!!")
                        else:
                            mycur.execute("select Id from ChemInv where Id={0};".format(Id))
                            log=mycur.fetchone()
                            if log is not None:
                                ...
                                mycur.execute("update ChemInv set qty=qty-{n} where Id={id};".format(n,Id))
                                mycon.commit()
        ...
        #BY Chemical name
                        if l==1:
                            ...
                            mycur.execute("select name from ChemInv where name=\"{0}\";".format(s))
                            ava=mycur.fetchone()
                            if ava is not None:
                                print("Chemical is in Stock!! ")
                                mycur.execute("select * from ChemInv where name=\"{0}\";".format(s))
                                x=mycur.fetchall()
                                for i in x:
                                    print(y)
                            else:
        ...
        #BY casnum
                            ...
                            g=int(input("Enter Cas no to search:"))
                            mycur.execute("select casnum from ChemInv where casnum=\"{0}\";".format(g))
                            ava=mycur.fetchall()
                            if ava is not None:
                                print("Chemical is in stock!! ")
                                mycur.execute("select * from ChemInv where casnum=\"{0}\";".format(g))
                                x=mycur.fetchall()
                                for i in x:
                                    print(y)
                            else:
                        ...
                        mycur.execute("select * from ChemInv order by name;")
                        v=mycur.fetchall()
                        for i in v:
                            print(i)
                    elif a==8:
                        mycur.execute("select name,qty,lstockqty from ChemInv where qty<(lstockqty+1);")
                        j=mycur.fetchall()
                        for i in j:
                            print(i)
                    elif a==9:
                        f=open("ChemInvData.csv",mode="w")
                        writer=csv.writer(f)
                        mycur.execute("select * from ChemInv;")
                        k=mycur.fetchall()
                        for i in k:
